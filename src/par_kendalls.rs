...
use kendalls::Error;
use rayon::{prelude::{IntoParallelRefIterator, IndexedParallelIterator, ParallelIterator, IntoParallelIterator}, slice::ParallelSliceMut};



pub fn tau_b_with_comparator<F>(x: &[f64], y: &[f64], mut comparator: F) -> Result<(f64, f64), Error>
where F: FnMut(&f64, &f64) -> Ordering {
    ...
    //let mut pairs: Vec<(f64, f64)> = x.par_iter().cloned().zip(y.par_iter().cloned()).collect();

    pairs.par_sort_by(|p1, p2| {
        let res = p1.0.partial_cmp(&p2.0).unwrap_or(Ordering::Greater);

        if res == Ordering::Equal {
            p1.1.partial_cmp(&p2.1).unwrap_or(Ordering::Greater)
        } else {
            res
        }
    });
