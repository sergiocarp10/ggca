...
impl ConstantInputError {
    ...
    fn warn_if_needed(&self) {
        let value = self.number_of_cor_filtered.lock().unwrap();
...
impl Analysis {
    ...
    fn run_analysis(
        ...
        let c1 = cross_product.collect_vec();
        let c_iter = c1.into_par_iter();

        let correlations_and_p_values = c_iter.map(|(tuple_1, tuple_2)| {
